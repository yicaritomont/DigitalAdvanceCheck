...
<div class="card-title">
    <h3 class="card-label">
        Roles de usuario
        <span class="d-block text-muted pt-2 font-size-sm">Cree, edite o elimine los roles utilizados en MDM</span>
    </h3>
</div>
...
</svg><!--end::Svg Icon--></span>	Nuevo Rol
</a>
<!--end::Button-->
</div>
</div>
            ...
            <tbody>
                @foreach($roles as $rol)
                    <tr>
                        ...
                        <td>{{$rol->name}}</td>
                        <td>
                        <form action="{{ route('roles.updateStatus', $rol->id) }}" method="POST">   
                            @csrf
                            @method('PUT')
                            @if($rol->status == 1)
                        ...
                        <td><a href="{{route('roles.edit', $rol->id)}}" class="btn btn-primary">Editar</a> </td>
                    </tr>
                @endforeach
            </tbody>
        </table>
